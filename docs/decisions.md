...
16- pdf ingestion, why hash per page instead of per file, and why reuse file_hash instead of a new table?

hashing the whole file was a bad idea once i actually thought about updates. if someone re-uploads a pdf where only one page changed, a file level hash changes too, and then i'd end up deleting and re embedding every chunk from every page again, even the ones that didnt change. thats wasted embedding calls and wasted db writes for no reason.

so instead i hash each page's extracted text on its own. i already had a file_hash column on document_chunks doing basically nothing useful for this, so i just repurposed it to mean page_hash, and added a page_number column next to it. every chunk that comes from the same page carries the same page_hash and page_number. on re-ingest i pull the pages already in the db for that source, and for each page in the new pdf i compare its hash to what's stored. if it matches i skip it, no re embedding. if it differs (or its a new page) i delete just that page's old chunks and add the new ones.

i thought about a seperate table to track page hashes instead of putting the columns directly on document_chunks, but that adds a join and a second table to keep in sync for no real benefit. the chunk count per page being more than 1 isnt caused by these 2 extra columns, its just the text splitter doing its job, so theres no conflict storing page_number/page_hash redundantly on every chunk row from that page.

i also hash per page and not per chunk, because chunk boundaries can shift slightly even when the actual page content hasnt really changed (splitter picks slightly different cut points), and that wouldve caused false "changed" detections on chunks that are basically the same content.

last part of this is orphaned pages. if someone re uploads a shorter version of the pdf, some page numbers that existed in the db before wont exist in the new file anymore. after processing all pages present in the new pdf, i take the page numbers stored in the db for that source, subtract the page numbers just seen in the new pdf, and delete chunks for whatever page numbers are left over. otherwise those old chunks just sit there forever polluting retrieval.

17- pdf text extraction, why PyMuPDF4LLM instead of plain PyMuPDF or Dolphin?

plain PyMuPDF's page.get_text() only gives raw text. no markdown, no real table structure, and multi column pdfs come out with mixed up reading order half the time. i was relying on this without realizing it wasnt even giving me proper structure, just plain text dumped in whatever order the pdf internally stores it.

i looked at a comparison across a few different pdf extraction libraries (PyMuPDF, PyMuPDF4LLM, Docling, PDF Extract Kit, Dolphin, dots.ocr, Amazon Textract) across how well each handles plain text, tables, multi column layout, and code blocks. Dolphin came out on top overall, its a vision based model from bytedance that detects layout first then parses content region by region, so it handles tables and figures really well. but its a proper model, it needs actual inference infra set up (gpu or hosted inference), which is way more than what this assignment needs right now.

PyMuPDF4LLM is a library built directly on top of PyMuPDF, still pip installable, no model to host. it fixes the exact problems plain PyMuPDF had, it reconstructs correct reading order for multi column pages and outputs proper markdown, headers, bold/italic, tables, code blocks all detected. it even has a page_chunks option that gives you back a list of dicts, one per page, each with its own markdown text and metadata already split out, which lines up exactly with the per page hashing i just built. its not as strong as dolphin on complex tables, but for the amount of infra it costs (basically none) its the right tradeoff here.